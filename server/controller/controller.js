...
exports.create=(req,res)=>{
    ...
    user
        ...
        .catch(err=>{
            res.status(500).send({
                message:err.message || "Some error occcured in create operation"
            })
        })
}
...
exports.find=(req,res)=>{
    ...
    if(req.query.id){
        ...
        .catch(err =>{
            res.status(500).send({message:"Error while retrieving user with the id" +id})
        })
    }
    else{
        Userdb.find()
        ...
        .catch(err =>{
            res.status(500).send({message:err.message || "Error occured while retrieving the information"})
        })
    }
...
exports.update=(req,res)=>{
    if(!req.body){
        ...
        .send({message:"Data to update cannot to be empty"})
    }
